...
class {
    ...
    init: \
    func ~empty (rows: UInt, columns: UInt) {
        elements = Float[rows][columns] new()
    }

    init: \
    func ~fromArrayOfArrays (elements: Float[][]) {
    ...
    add: \
    func ~individual (mat: Matrix) -> This {
    ...
    mul: \
    func ~byScalar (scalar: Float) -> This {
        for (i in 0 .. this rows) {
            for (j in 0 .. this columns) {
...
class {
    ...
    init: \
    func ~empty (dimensions: UInt) { components = Float[dimensions] new() }

    init: \
    func ~fromArray (=components)

    add: \
    func ~individual (vec: This) -> This {
    ...
    mul: \
    func ~individual (scalar: Float) -> This {
        for (i in 0 .. this dimensions) {
